...
class Zone:
    ...
    def draw_node(self, window: pygame.Surface, dx, dy):
        ...
        pygame.draw.circle(window, (220, 180, 255), Display.to_screen(x, y, dx, dy), 20)
        pygame.draw.circle(
            window, self.meta_data["color"], Display.to_screen(x, y, dx, dy), 15
        )

    def draw_edges(self, window: pygame.Surface, named_zones, dx, dy):
        start_pos = Display.to_screen(self.coordinates[0], self.coordinates[1], dx, dy)

        for connection in self.connections:
            color = graph_data.edge_colors[
                named_zones[connection["name"]].meta_data["zone"]
            ]
            end_pos = named_zones[connection["name"]].coordinates
            end_pos = Display.to_screen(end_pos[0], end_pos[1], dx, dy)
...
class Drone:
    def __init__(self, current_hub, end_hub, i):
        ...
        self.logs = []
        self.in_restricted = 0

    def draw_drone(self, screen, camera_x, camera_y):
        x, y = self.zone["coordinates"]
        screen_x, screen_y = Display.to_screen(x, y, camera_x, camera_y)
    ...
    def move_drone(self, graph, parse, screen, display) -> bool:
        ...
        for node in parse.zones:
            node.draw_edges(
                screen, parse.named_zones, display.camera_x, display.camera_y
            )
        for node in parse.zones:
        ...
        target_x = (
            self.drone_start_x
            + (self.target_coordinates[0] - self.drone_start_x) * progress
        )
        target_y = (
            self.drone_start_y
            + (self.target_coordinates[1] - self.drone_start_y) * progress
        )
    ...
    def find_next_move(self, parse, graph) -> None:
        ...
        sorted_connections = sorted(
            connections, key=lambda x: parse.named_zones[x["name"]].rank
        )

        min_rank = {"name": None, "rank": float("inf")}
        if sorted_connections:
            min_rank["name"] = sorted_connections[0]["name"]
            min_rank["rank"] = parse.named_zones[min_rank["name"]].rank

        if not min_rank["name"] or min_rank["rank"] == float("inf"):
            return

        best_node = parse.named_zones[min_rank["name"]]
        link_cap = graph.get_link_capacity(parse, self.zone["name"], best_node.name)
        link_use = graph.get_link_usage(self.zone["name"], best_node.name)

        if (
            best_node.drones_in < best_node.meta_data["max_drones"]
            ...
            if best_node.meta_data["zone"] == "restricted":
                self.logs.append(f"D{self.index}-{self.zone['name']}-{best_node.name}")
                self.in_restricted = 1
            else:
                self.logs.append(f"D{self.index}-{best_node.name}")
            return
        ...
        for connection in sorted_connections:
            ...
            if current.rank == min_rank["rank"] or current.rank == min_rank["rank"] + 1:
                ...
                link_cap = graph.get_link_capacity(
                    parse, self.zone["name"], current.name
                )
                link_use = graph.get_link_usage(self.zone["name"], current.name)

                if (
                    current.drones_in < current.meta_data["max_drones"]
                    and link_use < link_cap
                ):
                    parse.named_zones[self.zone["name"]].drones_in -= 1
                    current.drones_in += 1
                    ...
                    if current.meta_data["zone"] == "restricted":
                        self.logs.append(
                            f"D{self.index}-{self.zone['name']}-{current.name}"
                        )
                        self.in_restricted = 1
                    else:
                        self.logs.append(f"D{self.index}-{current.name}")
                    return
...
class Graph:
        ...
        while heap:
            ...
            for neighbor in current.connections:
                ...
                if neighbor not in visited:
                    ...
                    neighbor.rank = (
                        current.rank + graph_data.COST[neighbor.meta_data["zone"]]
                    )
    ...
    def find_next_hubs(self, parse, screen, display):
        ...
        for drone in self.moving_drones:
            ...
            target_x = (
                drone.drone_start_x
                + (drone.target_coordinates[0] - drone.drone_start_x) * progress
            )
            target_y = (
                drone.drone_start_y
                + (drone.target_coordinates[1] - drone.drone_start_y) * progress
            )
        ...
        for drone in self.drones:
            if (
                ...
                and drone.zone["name"] != parse.end_hub.name
            ):
                ...
                if drone.target:
                    ...
                    if drone.in_restricted == 1:
                        target_zone_coords = parse.named_zones[drone.target].coordinates
                        if started_new_move:
                            drone.target_coordinates = (
                                (drone.drone_start_x + target_zone_coords[0]) / 2,
                                (drone.drone_start_y + target_zone_coords[1]) / 2,
                            )
                        else:
                            drone.target_coordinates = target_zone_coords
                            drone.logs.append(f"D{drone.index}-{drone.target}")
                            drone.in_restricted = 0
                    else:
                        drone.target_coordinates = parse.named_zones[
                            drone.target
                        ].coordinates
                    ...
                    if started_new_move:
                        self.increment_link_usage(drone.zone["name"], drone.target)
